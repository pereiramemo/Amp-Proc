...
log_warn <- function(msg) {
  line <- paste0("[WARN] ", msg)
  message(line)
  log_lines[[length(log_lines) + 1]] <<- line
  warn_lines[[length(warn_lines) + 1]] <<- msg
}

log_error <- function(msg) {
  line <- paste0("[ERROR] ", msg)
  message(line)
  log_lines[[length(log_lines) + 1]] <<- line
}

# Assemble the standardized log file (general info + run log). The general-info
# block follows the format described in the project CLAUDE.md and must not
# contain any data statistics.
build_log <- function(sample_names, inputs, params, outputs, command,
                      exit_status, tool_log) {
  sep <- strrep("=", 75)
  c(
    sep, SCRIPT_NAME, sep,
    "",
    ...
    sprintf("- Sample names: %s", paste(sample_names, collapse = ", ")),
    "- Input data:",
    paste0("    ", inputs),
    "- Parameters:",
    paste0("    ", params),
    ...
    "",
    "Warnings:",
    if (length(warn_lines) > 0) paste0("    ", warn_lines) else "    None",
    "",
...
filename_nseq <- file.path(results_dir, "tables/nseq_counts.csv") # nolintr
...
filename_seqlength_stats <- file.path(results_dir, "tables/seq_length_stats.csv") # nolintr
...
filename_stats <- file.path(stats_dir, "2.1-dada2-pipeline-stats.tsv")
...
###############################################################################
### 22. Write standardized log (general info + run log)
###############################################################################
...
filename_log <- file.path(logs_dir, "2.1-dada2-pipeline.log")
...
log_text <- build_log(
  ...
  exit_status = 0,
  tool_log = gsub(ansi_re, "", log_lines)
)
