...
# Cutadapt requires reverse complement of reverse primer for paired-end trimming
# Using basic shell commands to create reverse complement
reverse_complement() {
    echo "$1" | tr 'ACGTacgtRYSWKMBDHVNryswkmbdhvn' 'TGCAtgcaYRSWMKVHDBNyrswmkvhdbn' | rev
}
...
log "Reverse primer (RC): ${PRIMER_REV_RC}"
...
echo -e "sample\ttotal_pairs\ttrimmed_pairs\tpercent_trimmed\ttotal_bp_r1_in\ttotal_bp_r1_out\ttotal_bp_r2_in\ttotal_bp_r2_out" > "${SUMMARY_FILE}"
...
for i in "${!R1_FILES[@]}"; do
    ...
    # -G for 5' adapter on R2 (reverse primer RC)
    CUTADAPT_CMD=(
        cutadapt
        -g "${PRIMER_FWD}"
        -G "${PRIMER_REV_RC}"
    ...
    PAIRS_WRITTEN=$(grep "Pairs written (passing filters):" "${LOG_FILE}" | awk '{print $NF}' | tr -d ',')
    TOTAL_BP_R1_IN=$(grep "Total basepairs processed:" "${LOG_FILE}" | head -1 | awk '{print $(NF-1)}' | tr -d ',')
    TOTAL_BP_R1_OUT=$(grep "Total written (filtered):" "${LOG_FILE}" | head -1 | awk '{print $(NF-1)}' | tr -d ',')
    TOTAL_BP_R2_IN=$(grep "Total basepairs processed:" "${LOG_FILE}" | tail -1 | awk '{print $(NF-1)}' | tr -d ',')
    TOTAL_BP_R2_OUT=$(grep "Total written (filtered):" "${LOG_FILE}" | tail -1 | awk '{print $(NF-1)}' | tr -d ',')
    ...
    echo -e "${SAMPLE_NAME}\t${TOTAL_PAIRS:-0}\t${PAIRS_WRITTEN:-0}\t${PERCENT_TRIMMED}\t${TOTAL_BP_R1_IN:-0}\t${TOTAL_BP_R1_OUT:-0}\t${TOTAL_BP_R2_IN:-0}\t${TOTAL_BP_R2_OUT:-0}" >> "${SUMMARY_FILE}"
...
Reverse primer (RC):    ${PRIMER_REV_RC}
